...
if __name__ == '__main__':
    ...
    looper = wrap_looper(SystemClass=None, params=params, func=func_rat_var_min, looper='XYLooper', file_path_prefix='data/v1.4-qom-v0.9.0/5_n=10')
    # looper = run_loopers_in_parallel(SystemClass=None, params=params, func=func_rat_var_min, looper='XYLooper', file_path_prefix='data/v1.4-qom-v0.9.0/5_n=10')
    xs = looper.axes['Y']['val']
    vs_0 = list()
    for V in looper.results['V']:
        _, var_0 = np.transpose(V).tolist()
        vs_0.append(np.min(var_0))

    # high thermal phonons
    params['system']['ns'][1] = 1000.0
    looper = wrap_looper(SystemClass=None, params=params, func=func_rat_var_min, looper='XYLooper', file_path_prefix='data/v1.4-qom-v0.9.0/5_n=1000')
    # looper = run_loopers_in_parallel(SystemClass=None, params=params, func=func_rat_var_min, looper='XYLooper', file_path_prefix='data/v1.4-qom-v0.9.0/5_n=1000')
    vs_1 = list()
    for V in looper.results['V']:
        _, var_1 = np.transpose(V).tolist()
        vs_1.append(np.min(var_1))
