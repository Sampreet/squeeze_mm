...
if __name__ == '__main__':
    ...
    looper = wrap_looper(SystemClass=None, params=params, func=func_rat_var, looper='XYLooper', file_path_prefix='data/v2.2-qom-v0.9.0/5_rwa_n=10')
    xs = looper.axes['Y']['val']
    vs_0_ft = list()
    vs_0_rwa = list()
    for V in looper.results['V']:
        rat, m_ft, m_min, m_ss = np.transpose(V).tolist()
        vs_0_ft.append(np.min(m_ft))
        vs_0_rwa.append(np.min(m_min))

    # high thermal phonons with rwa
    params['system']['ns'][1] = 1000.0
    params['system']['t_rwa'] = True
    looper = wrap_looper(SystemClass=None, params=params, func=func_rat_var, looper='XYLooper', file_path_prefix='data/v2.2-qom-v0.9.0/5_rwa_n=1000')
    xs = looper.axes['Y']['val']
    vs_1_ft = list()
    vs_1_rwa = list()
    for V in looper.results['V']:
        rat, m_ft, m_min, m_ss = np.transpose(V).tolist()
        vs_1_ft.append(np.min(m_ft))
        vs_1_rwa.append(np.min(m_min))
    ...
    plotter.update(xs=xs, vs=- 10 * np.log10([vs_0_ft, vs_1_ft, [0.5] * len(vs_0_ft)]))
    ax = plotter.get_current_axis()
    ax.scatter(xs[::3], - 10 * np.log10(vs_0_rwa)[::3], c='b', s=50)
    ax.scatter(xs[::3], - 10 * np.log10(vs_1_rwa)[::3], c='r', s=50)
    plotter.show(True)